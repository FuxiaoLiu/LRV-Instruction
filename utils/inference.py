...
if __name__ == "__main__":
    ...
    parser.add_argument("--bf16", default=False, action="store_true", help="using 8bit mode")
    ...
    #image folder
    image_folder = '/fs/vulcan-projects/semantic_consistency_fuxiao/MSR/MME_Benchmark_release_version/landmark/images/images/'

    #insert your data file right here
    #datas should be the json format like:
    datas = [{'instruction': "", 'image_id': "", 'gt': ""}]
  
    result = []
    for data1 in tqdm(datas):
        instruction = data1['instruction']
        prompt = "The following is a conversation between a curious human and AI. The AI gives helpful, detailed, and polite answers to the human's questions.\nHuman: <image>\nHuman: " + instruction + "\nAI:"
        image_path = data1['image_id']
        ...
        data_format = {
            ...
            "max_new_tokens": 512,}}
        output_text = model.predict1(data_format)

        result.append(data1['image_id']+'\t'+data1['instruction']+'\t'+data1['gt']+'\t'+output_text)

     #Then save the result into the json or txt file
